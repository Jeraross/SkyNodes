...
def _garantir_adjacencias(csv_aeroportos: str, csv_adjacencias: str) -> None:
...
def _validar_iata(grafo, iata: str, papel: str) -> None:
    """
    Valida que o IATA existe no grafo.
    Lança SystemExit com mensagem clara se não existir.
    """
...
def _salvar_json(dados: dict, caminho: str) -> None:
    """Serializa o dicionário de resultado em JSON, convertendo tipos não serializáveis."""
    os.makedirs(Path(caminho).parent, exist_ok=True)

    def _converter(obj):
        # float('inf') não é JSON válido; converte para string
...
def _rodar_bfs(grafo, source: str, diretorio_saida: str) -> None:
    ...
    _salvar_json(resultado_serial, caminho)
...
def _rodar_dijkstra(grafo, source: str, target: str, diretorio_saida: str) -> None:
    ...
    _salvar_json(resultado_serial, caminho_arq)
...
def main() -> None:
    ...
    if alg == "ALL":
        _garantir_adjacencias(csv_aeroportos, csv_adjacencias)
